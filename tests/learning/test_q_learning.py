...
@pytest.fixture
def q_learning_n1():
    return QLearning(learning_rate=0.1, discount_factor=0.9, default_q_value=1.0, n_steps=1)

@pytest.fixture
def q_learning_n3():
    return QLearning(learning_rate=0.1, discount_factor=0.9, default_q_value=1.0, n_steps=3)
...
@pytest.mark.unit
def test_update_n_step_1(q_learning_n1, trajectory):
    expected_q_value = 1.0 + 0.1 * (3 + 0.9 * 1.0 - 1.0)  # current_q_value + learning_rate * (reward + discount_factor * max_q_value - current_q_value)
    
    q_learning_n1.update(trajectory)
    
    updated_q_value = q_learning_n1.q_values[trajectory.transitions[-1].state][trajectory.transitions[-1].action]
    assert updated_q_value == pytest.approx(expected_q_value, abs=1e-6)

@pytest.mark.unit
def test_update_n_step_3(q_learning_n3, trajectory):    
    expected_q_value = 1.0 + 0.1 * (1 + 0.9 * 2 + 0.9**2 * 3 + 0.9**3 * 1.0 - 1.0)  # current_q_value + learning_rate * (sum of discounted rewards - current_q_value)
    
    q_learning_n3.update(trajectory)
    
    updated_q_value = q_learning_n3.q_values[trajectory.transitions[0].state][trajectory.transitions[0].action]
    assert updated_q_value == pytest.approx(expected_q_value, abs=1e-6)

@pytest.mark.unit
def test_update_terminal_state_n_step_3(q_learning_n3, trajectory):
    ...
    updated_q_value = q_learning_n3.q_values[trajectory.transitions[1].state][trajectory.transitions[1].action]
    assert updated_q_value == pytest.approx(expected_q_value_s2, abs=1e-6)

    updated_q_value = q_learning_n3.q_values[trajectory.transitions[2].state][trajectory.transitions[2].action]
