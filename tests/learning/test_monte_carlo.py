...
from learning import MonteCarlo
from agents import Trajectory, Transition
from environment import Action

@pytest.fixture
def monte_carlo():
    return MonteCarlo(learning_rate=0.1, discount_factor=0.9, default_q_value=1.0)
...
@pytest.mark.unit
def test_update_complete_trajectory(monte_carlo, trajectory):    
    # Calculate expected Q-values for each state-action pair in monte carlo
    ...
    monte_carlo.update(trajectory)
    
    updated_q_value = monte_carlo.q_values[trajectory.transitions[0].state][trajectory.transitions[0].action]
    assert updated_q_value == pytest.approx(expected_q_value_s1, abs=1e-6)

    updated_q_value = monte_carlo.q_values[trajectory.transitions[1].state][trajectory.transitions[1].action]
    assert updated_q_value == pytest.approx(expected_q_value_s2, abs=1e-6)

    updated_q_value = monte_carlo.q_values[trajectory.transitions[2].state][trajectory.transitions[2].action]
    assert updated_q_value == pytest.approx(expected_q_value_s3, abs=1e-6)

@pytest.mark.unit
def test_update_incomplete_trajectory(monte_carlo, trajectory):
    # Modify trajectory to be incomplete
    trajectory.transitions[-1].is_terminal = False
    
    monte_carlo.update(trajectory)
    
    # Ensure Q-values are not updated
    for transition in trajectory.transitions:
        assert monte_carlo.q_values[transition.state][transition.action] == 1.0