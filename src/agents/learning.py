...
class MonteCarlo(LearningAlgorithm):
    ...
    def update(self, trajectory: Trajectory) -> None:
        ...
        for transition in reversed(trajectory.transitions):
            ...
            self.q_values[state][action] += (cumulative_reward - self.q_values[state][action]) / self.visit_counts[
                state
            ][action]
...
class QLearning(LearningAlgorithm):
    ...
    def _update_q_values(self, n_step_trajectory: Trajectory, is_terminal: bool) -> None:
        ...
        if not is_terminal:
            ...
            max_q_value = max(self.q_values[next_state][action] for action in Action)
...
class AlgorithmFactory:
    LEARNING_ALGORITHMS = {Sarsa.__name__: Sarsa, QLearning.__name__: QLearning, MonteCarlo.__name__: MonteCarlo}
